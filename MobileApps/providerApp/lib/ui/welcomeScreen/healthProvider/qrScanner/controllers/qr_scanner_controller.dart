...
class QrScannerController extends GetxController {
  ...
  void onInit() {
    super.onInit();
    addServerCount = 0;
    Debug.printLog("on Init Qr Scanner Controller server Count $addServerCount  ${Preference.shared.getBool(Preference.isSingleServer) ?? false}");
    if (argument != null) {
      if (argument[0] != null) {
  ...
  void onChangeUrl(int index) {
    ...
    if(Utils.getServerList.where((element) => element.isSelected).toList().length == 1){
      ...
      // Preference.shared.setString(Preference.patientId, Utils.getServerList.where((element) => element.isSelected).toList()[0].patientId);
    }
  }
  ...
  Future<void> addNewServerUrlDataIntoList(bool isSelectedItem,setStateDialogForChooseCode,{bool isNotClose = true,bool isEditServer = false,int index = -1}) async {
    if(addOtherUrlController.text.isNotEmpty) {
      ...
      if(isAuth) {
        ...
        addOtherUrlController.clear();
        addServerTitleName.clear();
        if(Preference.shared.getBool(Preference.isSingleServer) ?? false){
         gotoNextPage();
        }else {
          showDialogAfterSuccess();
        }
  ...
  addNewServerUrlAuth(bool isSelectedItem,setStateDialogForChooseCode,{bool isNotClose = true,int index = -1,bool isEditServer = false}) async {
    if(addOtherUrlController.text.isNotEmpty) {
      ...
      var serverData = ServerModelJson(
          ...
          isPrimary: false,
          title: addServerTitleName.text.toString(),
          isSingleServer: Preference.shared.getBool(Preference.isSingleServer) ?? false);
      if(!isEditServer) {
        if (Utils.getServerList
            .where((element) =>
        element.displayName == serverData.displayName &&
      ...
      if(Utils.getServerList.where((element) => element.isSelected).toList().where((element) => element.authToken == ""
        ...
        if (isFromSetting) {
          ...
          if(Utils.getServerList.where((element) => element.isSelected).toList().length > 1) {
            ...
            if(Utils.getServerList.where((element) => element.isSelected).where((element) => element.providerId == "").toList().isEmpty){
              Debug.printLog(".....${Utils.getServerList.where((element) => element.isSelected).where((element) => element.providerId == "").toList().isEmpty}");
              ...
              if(Utils.getServerList.where((element) => element.isSelected && element.patientId == "").toList().isNotEmpty){
                Preference.shared.setBool(Constant.keyWelcomeDetails,false);
                Preference.shared.setBool(Constant.keyIndependentPatient,true);
                Get.toNamed(AppRoutes.patientIndependentMode,arguments: [false,true,false,false]);
              }else{
                  Get.back();
                  Get.back();
              }
              ...
              if(Utils.getServerList.where((element) => element.isSelected).toList().length > 1 && Utils.getServerList.where((element) =>  element.providerId != "").toList().isNotEmpty){
          ...
          if(Utils.getServerList.where((element) => element.isSelected).toList().length > 1) {
            ...
            if(Utils.getServerList.where((element) =>  element.providerId != "").toList().isNotEmpty){
              Get.toNamed(AppRoutes.patientIndependentMode, arguments: [false,false,false,false]);
              Preference.shared.setBool(Constant.keyWelcomeDetails,false);
              Preference.shared.setBool(Constant.keyIndependentPatient,true);
            }
  ...
  Future<void> callSecureServerAPI(String url,String clientId,String serverName, setStateDialogForChooseCode,{Function? callBack}) async {
    if (url.isNotEmpty) {
      ...
      try {
        ...
        if (clientId != "") {
          ...
          if(!isSuccess && fhirParameters.isEmpty){
            ...
            addServerTitleName.clear();
            // Utils.showToast(Get.context!, );
            // Utils.showErrorDialog(Get.context!,Constant.txtError,Constant.txtUserCanceled);
            if(callBack != null) {
              var returnValue = {};
          ...
          if(getIndexOfSecureServer != -1) {
            ...
            serverData[getIndexOfSecureServer].lastDateTimeStr =
                DateTime.now().toString();
            serverData[getIndexOfSecureServer].lastLoggedTime = DateTime
                .now()
            ...
            serverData[getIndexOfSecureServer].providerFName =
                smartFhirProviderDisplayName;
            serverData[getIndexOfSecureServer].providerLName =
                smartFhirProviderDisplayName;
            serverData[getIndexOfSecureServer].isAdmin = isAdmin;
            serverData[getIndexOfSecureServer].patientId = "";
            serverData[getIndexOfSecureServer].patientFName = "";
            serverData[getIndexOfSecureServer].patientLName = "";
            serverData[getIndexOfSecureServer].patientDOB = "";
            serverData[getIndexOfSecureServer].patientGender = "";
                      ...
                      child: Container(
                        ...
                        child: Column(
                          children: [
                                    ...
                                    Container(
                                      ...
                                      child: Row(
                                        children: [
                                          Text(
                                            "Connection Name",
                                            style: AppFontStyle.styleW700(CColor.black,FontSize.size_3 ),
                                          ),
                                        ],
                                      ),
                                    ),
                            ...
                            Container(
                              margin: EdgeInsets.only(
                                  right: Sizes.width_1, left: Sizes.width_1),
                              child: Row(
                                children: [
                                  Text(
                                    "Connection URL",
...
/*  showDialogAddNewCamera() {
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                child: Text("Authorized?",
                                  style: TextStyle(color: CColor.black,fontSize: FontSize.size_10),),
                              )
                            ],
                          ),
                          Container(
                              ...
                              children: [
                                ...
                                Expanded(
                                  child: TextButton(
                                    onPressed: () {
                                      if(isAuth){
                                        addNewServerUrlAuth(true,Get.context!);
                                      }else{
                                        addNewServerUrlDataIntoList(
                                            true,Get.context!);
                                      }
  ...
  Future<void> getProviderList(bool isFromSearch) async {
    ...
    var selectedUrlModel = ServerModelJson(isSecure: false,
        ...
        isSingleServer: Preference.shared.getBool(Preference.isSingleServer) ?? false);
    selectedUrlModel.isAdmin = false;
                    ...
                    return Column(
                      ...
                      children: [
                        Padding(
                          ...
                          child: Row(
                            children: [
                              Expanded(child: _widgetSearchIDText(constraints)),
                              Expanded(child: _widgetSearchNameText(constraints)),
                            ],
                          ),
                        ),
  ...
  Widget _widgetSearchIDText(BoxConstraints constraints) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Container(
  ...
  Widget _widgetSearchNameText(BoxConstraints constraints) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Container(
  ...
  _itemPatientUser(int index, BuildContext context, PatientDataModel patientProfileList,BoxConstraints constraints) {
    return GestureDetector(
      onTap: () {
        ...
        if(serverModelDataList.isNotEmpty){
          var data = serverModelDataList.where((element) => element.isSelected && !element.isSecure
          ...
          if(data.isNotEmpty) {
            ...
            /// Name In Use Patient But Show Provider
            serverModelDataList[serverIndex].providerId =
                patientProfileList.patientId;
            data[0].providerId = patientProfileList.patientId;
            data[0].providerFName = patientProfileList.fName;
            data[0].providerLName = patientProfileList.lName;
            data[0].providerDOB = patientProfileList.dob;
            data[0].providerGender = patientProfileList.gender;
            data[0].isSingleServer = Preference.shared.getBool(Preference.isSingleServer) ?? false;
            if(Preference.shared.getBool(Preference.isSingleServer) ?? false){
              data[0].isPrimary = true;
            }
            // data[0].patientDOB = patientProfileList.dob;
            // data[0].patientGender = patientProfileList.gender;
            serverModelDataList[serverIndex] = data[0];
            var json = jsonEncode(
                serverModelDataList.map((e) => e.toJson()).toList());
            Preference.shared.setList(Preference.serverUrlAllListed, json);
            ...
            Get.back();
            if(Preference.shared.getBool(Preference.isSingleServer) ?? false){
              Utils.showToast(Get.context!, Constant.successFullyConnected);
             gotoNextPage();
            }else {
              showDialogAfterSuccess();
            }
