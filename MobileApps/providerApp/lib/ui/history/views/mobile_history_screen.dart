...
class MobileHistoryScreen extends StatelessWidget {
      ...
      body: SafeArea(
        child: GetBuilder<HistoryController>(
          ...
          builder: (logic) {
            ...
            // return  Utils.pullToRefreshApiTrackingChart(_widgetTackingScreen(orientation,logic,context),
            //     logic.refreshController, logic.onRefresh, logic.onLoading);
            return _widgetTackingScreen(orientation,logic,context);
          },
        ),
      ),
    );
  ...
  _widgetTackingScreen(Orientation orientation, HistoryController logic, BuildContext context){
    ...
    Column(
      ...
      children: [
        ...
        // Expanded(child: _widgetUserData(context,orientation,logic))
      ],
    ):Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
...
/*        htdRefreshController: logic.refreshController,
        onLoad: logic.onLoading,
        onRefresh: logic.onRefresh,
        refreshIndicator: WaterDropMaterialHeader(
        ...
        rightHandSideColumnWidth: MediaQuery.of(context).size.width * (Utils.getTableWidth(
            ...
            (logic.trackingPrefList.where((element) => element.titleName == Constant.configurationExperience
                && element.isSelected).toList().isNotEmpty),logic)),
        isFixedHeader: true,
  ...
  List<Widget>? headerWidget(
      Orientation orientation, HistoryController logic, BuildContext context) {
    List<Widget> header = [];
    ...
    for (int i = 0; i < logic.trackingPrefList.length; i++) {
      if (logic.trackingPrefList[i].titleName ==
          Constant.configurationHeaderModerate && logic.trackingPrefList[i].isSelected) {
        header.add(cActivityMinNormal(orientation,logic,context,Constant.configurationHeaderModerate));

      } else if (logic.trackingPrefList[i].titleName ==
          Constant.configurationHeaderVigorous && logic.trackingPrefList[i].isSelected) {
        header.add(cActivityMinNormal(orientation,logic,context,Constant.configurationHeaderVigorous));

      } else if (logic.trackingPrefList[i].titleName ==
          Constant.configurationHeaderTotal && logic.trackingPrefList[i].isSelected) {
        header.add(cActivityMinNormal(orientation,logic,context,Constant.configurationHeaderTotal));

      } else if (logic.trackingPrefList[i].titleName ==
          Constant.configurationNotes && logic.trackingPrefList[i].isSelected) {
        header.add(cActivityMinNormal(orientation,logic,context,Constant.configurationNotes));

      } else if (logic.trackingPrefList[i].titleName ==
          Constant.configurationHeaderDays && logic.trackingPrefList[i].isSelected) {
        header.add(cDaysStrNormal(orientation,logic,context));

      } else if (logic.trackingPrefList[i].titleName ==
          Constant.configurationHeaderCalories && logic.trackingPrefList[i].isSelected) {
        header.add(cCaloriesNormal(orientation,logic,context));

      } else if (logic.trackingPrefList[i].titleName ==
          Constant.configurationHeaderSteps && logic.trackingPrefList[i].isSelected) {
        header.add(cStepsNormal(orientation,logic,context));

      } else if (logic.trackingPrefList[i].titleName ==
          Constant.configurationHeaderRest && logic.trackingPrefList[i].isSelected) {
        header.add(cHeartRateNormal(orientation,logic,context,Constant.configurationHeaderRest));

      } else if (logic.trackingPrefList[i].titleName ==
          Constant.configurationHeaderPeck && logic.trackingPrefList[i].isSelected) {
        header.add(cHeartRateNormal(orientation,logic,context,Constant.configurationHeaderPeck));

      } else if (logic.trackingPrefList[i].titleName ==
          Constant.configurationExperience && logic.trackingPrefList[i].isSelected) {
        ...
        children: [
          SizedBox(
            width:MediaQuery.of(contex).size.width *
                ((columnType == Constant.configurationHeaderModerate)?Utils.getModRowColumnWidth(contex,logic,isFromHeader: false):
                (columnType == Constant.configurationHeaderVigorous)?Utils.getModRowColumnWidth(contex,logic,isFromHeader: false):
                (columnType == Constant.configurationHeaderTotal)?Utils.getTotalRowColumnWidth(contex,logic,isFromHeader: false):
                Utils.getNotesRowColumnWidth(contex,logic,isFromHeader: false)),
  ...
  Widget _widgetDaysStrengthDataList(Orientation orientation, HistoryController logic,BuildContext contex){
    return Container(
      ...
      child: Row(
        ...
        children: [
          Container(
            width: MediaQuery.of(contex).size.width * Utils.getDaysStrengthRowColumnWidth(contex,logic,isFromHeader: false),
            alignment: Alignment
                .topCenter,
            child: ListView.builder(
              itemBuilder: (context,
  ...
  Widget _widgetCaloriesDataList(Orientation orientation, HistoryController logic,BuildContext contex){
    return Container(
      ...
      child: Row(
        ...
        children: [
          Container(
            width:MediaQuery.of(contex).size.width * Utils.getCaloriesRowColumnWidth(contex,logic,isFromHeader: false),
            alignment: Alignment
                .topCenter,
            child: ListView.builder(
              itemBuilder:
...
/*        const VerticalDivider(
  ...
  Widget  _widgetStepsDataList(Orientation orientation, HistoryController logic,BuildContext contex){
    return Container(
      ...
      child: Row(
        ...
        children: [
          Container(
            width: MediaQuery.of(contex).size.width * Utils.getStepsRowColumnWidth(contex,logic,isFromHeader: false),
            alignment: Alignment
                .topCenter,
            child: ListView.builder(
              itemBuilder:
  ...
  Widget _widgetHeartRatePeakDataList(Orientation orientation, HistoryController logic,BuildContext contex,
      String columnName){
    return Container(
      decoration: const BoxDecoration(
          border: Border(
      ...
      child: Row(
        ...
        children: [
          Container(
            width: MediaQuery.of(contex).size.width * Utils.getPeakHeartRateRowColumnWidth(contex, logic,isFromHeader: false),
            alignment: Alignment
                .topCenter,
            child: ListView.builder(
              itemBuilder:
                    ...
                    Expanded(
                      child: Container(
                        ...
                        child: editablePeakWeek(
                            mainIndex, logic, heartRatePeak,logic.trackingPrefList,context,
                            onChangeData: (value) {
                            }),
                      ),
                    ),
  ...
  _itemHeartRatePeakDay(int daysIndex,
      ...
      int mainIndex,
      String titleType, String columnName) {
    return (logic.trackingChartDataList[mainIndex].dayLevelDataList[daysIndex].isShow)?
    Column(
      children: [
        Column(
          children: [
            ...
            Container(
                ...
                child: Row(
                  children: [
                    if (logic.trackingPrefList.where((element) =>
                    element.titleName == Constant.configurationHeaderPeck &&
                        element.isSelected).toList().isNotEmpty && columnName == Constant.configurationHeaderPeck)
                      Expanded(
                        child: Container(
                          padding: EdgeInsets.only(
                            bottom: Sizes.height_1,
                          ...
                          child: editablePeakDay(
                              mainIndex, logic, heartRatePeak[daysIndex],logic.trackingPrefList,context,
                              onChangeData: (value) {
                              }),
                        ),
                      ),
  ...
  Widget _widgetHeartRateRestDataList(Orientation orientation, HistoryController logic,BuildContext contex,
      String columnName){
    return Container(
      decoration: const BoxDecoration(
          border: Border(
      ...
      child: Row(
        ...
        children: [
          Container(
            width: MediaQuery.of(contex).size.width * Utils.getRestHeartRateRowColumnWidth(contex, logic,isFromHeader: false),
            alignment: Alignment
                .topCenter,
            child: ListView.builder(
              itemBuilder:
                    ...
                    Expanded(
                      child: Container(
                        ...
                        child: editableRestWeek(
                            mainIndex, logic, heartRateRest,logic.trackingPrefList,context,
                            onChangeData: (value) {
                            }),
                      ),
                    ),
            ...
            Container(
                ...
                child: Row(
                  children: [
                    if (logic.trackingPrefList.where((element) =>
                    element.titleName == Constant.configurationHeaderRest &&
                        element.isSelected).toList().isNotEmpty && columnName == Constant.configurationHeaderRest)
                      Expanded(
                        child: Container(
                          padding: EdgeInsets.only(
                            bottom: Sizes.height_1,
                          ...
                          child: editableRestDay(
                              mainIndex, logic, heartRateRestDataList[daysIndex],logic.trackingPrefList,context,
                              onChangeData: (value) {
                              }),
                        ),
                      ),
  ...
  Widget _widgetExperienceDataList(Orientation orientation, HistoryController logic,BuildContext contex){
    return  Container(
      ...
      child: Row(
        ...
        children: [
          Container(
            width:MediaQuery.of(contex).size.width * Utils.getExperienceRowColumnWidth(contex,logic,isFromHeader: false),
            alignment: Alignment
                .topCenter,
            child: ListView.builder(
              itemBuilder:
  ...
  _itemWhatWhenWeek(int mainIndex, BuildContext context, HistoryController logic,
                    ...
                    Container(
                      ...
                      child: Text(
                        "${DateFormat(Constant.commonDateFormatMmmDd).format(
                            ...
                            data.weekEndDate!)}",
                        style: TextStyle(
                            fontWeight: (logic.trackingChartDataList[mainIndex].isExpanded)
                                ? FontWeight.w700
                                : FontWeight.normal),
                      ),
                    ),
    ...
    _itemWhatWhenDay(int daysIndex, BuildContext context,
      List<DayLevelData> weekDaysDataList, HistoryController logic, int mainIndex,Orientation orientation) {
    return (logic.trackingChartDataList[mainIndex].dayLevelDataList[daysIndex].isShow)?
    Column(
      children: [
        Container(
              ...
              child: Row(
                ...
                children: [
                  Container(
                    ...
                    /*child: Icon(
                      Icons.close,
                      color: (Constant.isEditMode) ? CColor.transparent : Colors
                          .transparent,
                      size: Sizes.width_4,
                    ),*/
                  ),
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Container(
                            ...
                            child: Text(
                              weekDaysDataList[daysIndex].activityLevelDataList[daysDataIndex].displayLabel.toString(),
  ...
  Future<void> selectFromAndToDate(BuildContext context,DateTime startDate,DateTime endDate,
                ...
                children: [
                  AlertDialog(
                    ...
                    content: Column(
                      ...
                      children: <Widget>[
                        Container(
                          margin: EdgeInsets.only(top: Sizes.height_2,left: Sizes.width_4),
                          child: Text(
                            activityName,
                            style: TextStyle(
                        ...
                        Container(
                          margin: EdgeInsets.symmetric(vertical: Sizes.height_2,horizontal: Sizes.width_4),
                          child: Row(
                            children: [
                              const Text('Starts'),
                              // const Spacer(),
                              Expanded(
                                child: GestureDetector(
                                  onTap: () {
                                    /*selectDateTime(context, false,startDate, endDate,(dateTime){
                                      setState((){
                                        startDate = dateTime;
                                  ...
                                  child: Container(
                                    ...
                                    child: Text(
                                      ...
                                      DateFormat.yMMMd().add_jm().format(startDate),
                        ...
                        Container(
                          margin: EdgeInsets.only(bottom: Sizes.height_2,right: Sizes.width_4),
                          child: Row(
                            children: [
                              const Spacer(),
                              GestureDetector(
                                onTap: () {
                                    logic.resetLastDate(weekDaysDataList,daysIndex,daysDataIndex,startDate
                                    ,endDate);
                                },
                                child: Container(
  ...
  Future<void> selectDateTime(BuildContext context,bool isEndDate,DateTime fromDate,DateTime toDate,
  ...
  _itemActivityMinWeek(int mainIndex, BuildContext context, WeekLevelData dataList,
            ...
            children: [
              ...
              element.titleName == Constant.configurationNotes &&
                ...
                child: InkWell(
                  ...
                  child: Container(
                    alignment: Alignment.center,
                    child: (logic.trackingChartDataList[mainIndex].weeklyNotes.isNotEmpty) ?Image.asset(
  ...
  _itemActivityMinDay(int daysIndex, BuildContext context,
      List<DayLevelData> weekDaysDataList, HistoryController logic, int mainIndex, String columnType) {
    return (logic.trackingChartDataList[mainIndex].dayLevelDataList[daysIndex].isShow)?
    Column(
      children: [
        Column(
                ...
                children: [
                  ...
                  Expanded(
                    child: Container(
                      ...
                      }, mainIndex, daysIndex, logic,Constant.activityMinutesMod,logic.trackingPrefList,context),
                    ),
                  ):Container(),
                  const Text("  "),
                  ...
                  Expanded(
                    child: Container(
                      ...
                      }, mainIndex, daysIndex, logic,Constant.activityMinutesVig,logic.trackingPrefList,context),
                    ),
                  ):Container(),
                  const Text("  "),
                  ...
                  if(logic.trackingPrefList.where((element) => element.titleName == Constant.configurationHeaderTotal
                    ...
                    child: Container(
                      ...
                      }, mainIndex, daysIndex, logic,Constant.activityMinutesTotal,logic.trackingPrefList,context),
                    ),
                  ),
                  ...
                  element.titleName == Constant.configurationNotes &&
                    ...
                    child: InkWell(
                      onTap: () {
                        /*if(Constant.isEditMode && Constant.configurationInfo.where((element) => element.isNotes ).toList().isNotEmpty) {
                          logic.setNotesOnController(logic.trackingChartDataList[mainIndex]
                      ...
                      child: Container(
                        ...
                        child:
                          (logic.trackingChartDataList[mainIndex]
                              .dayLevelDataList[daysIndex].dailyNotes.isNotEmpty)?Image.asset(
                            "assets/icons/ic_comment.png",
                            height: Sizes.width_4,
                            width: Sizes.width_4,
                            // color: Colors.grey,
                  ...
                  if(logic.trackingPrefList.where((element) => element.titleName == Constant.configurationHeaderModerate
                    ...
                    child: Container(
                      ...
                      child: editableActivityMinModDayData(
                          onChangeData: (value) {
                          }, mainIndex, daysIndex, daysDataIndex, logic,context),
                    ),
                  ),
                  const Text("  "),
                    ...
                    child: InkWell(
                      onTap: () {
                        ...
                        //     element.isNotes ).toList().isNotEmpty){
                          logic.setNotesOnController(logic.trackingChartDataList[mainIndex]
                              .dayLevelDataList[daysIndex].activityLevelDataList[daysDataIndex].dayDataNotes);
                      ...
                      child: Container(
                        ...
                        child:
                          (logic.trackingChartDataList[mainIndex]
                              .dayLevelDataList[daysIndex].activityLevelDataList[daysDataIndex].dayDataNotes.isNotEmpty)?Image.asset(
                            "assets/icons/ic_comment.png",
                            height: Sizes.width_4,
                            width: Sizes.width_4,
                            // color: Colors.grey,
          ...
          Container(
            // height: Sizes.height_6,
              ...
              child: Container(
                ...
                child: editableCalStepWeek(mainIndex, logic, titleType,dataList,logic.trackingPrefList,context,
                    onChangeData: (value) {
              ...
              // height: Sizes.height_7,
                ...
                child: Container(
                  ...
                  child: editableCalStepDay(
                      ...
                      onChangeData: (value) {}, mainIndex, titleType,logic.trackingPrefList,context),
                )
            ),
          ],
        ),
  ...
  Widget _itemExWeek(
      ...
      int mainIndex, List<CaloriesStepHeartRateWeek> dataList, HistoryController logic,
            ...
            Row(
              children: [
                Expanded(
                  child: InkWell(
                    onTap: (){
                      FocusScope.of(context).requestFocus(dataList.weekValueTitle2CheckBoxFocus);
                    },
                    child: Container(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        HtmlWidget(
                          ...
                          textStyle:  TextStyle(fontSize: FontSize.size_10),
                        ),
                      ),
                      Container(
                        padding: EdgeInsets.only(
                          ...
                          children: [
                            Expanded(
                              child: InkWell(
                                ...
                                child: Container(
                                  margin: EdgeInsets.only(left: Sizes.width_5,right: Sizes.width_4,),
                                  alignment: Alignment.center,
                                  decoration: BoxDecoration(
                                      border: Border.all(),
                                      borderRadius: BorderRadius.circular(10)),
