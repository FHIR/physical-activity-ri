...
/*  static HealthFactory health = HealthFactory(useHealthConnectIfAvailable: true);
  ...
  // static importDataFromHealth(Function callBack,bool isShow,{bool needAPICall = false}) async {
  //   try {
  //     var permissions = ( (Platform.isAndroid)?Utils.getAllHealthTypeAndroid:Utils.getAllHealthTypeIos).map((e) => HealthDataAccess.READ_WRITE).toList();
  //
  //     // bool? hasPermissions =
  ...
  //             var activityType = activityTypeData[i].value
  //                 .toJson()['workoutActivityType'];
  //             var totalCalories = activityTypeData[i].value
  //                 .toJson()['totalEnergyBurned'];
  //             totalCalories ??= "0";
  ...
  //         activityTypeData = healthData.where((element) => element.type == HealthDataType.WORKOUT).toList();
  //         for (int i = 0; i < activityTypeData.length; i++) {
  //           var id = await Utils.getPackageName();
  //           // if (activityTypeData[i].sourceId == id) {
  //             var activityType = activityTypeData[i].value
  ...
  static getWorkoutData(yesterday,today) async {
    List<HealthDataPoint> healthData = [];
    healthData = await health.getHealthDataFromTypes(yesterday, today, [HealthDataType.WORKOUT]);
    return healthData;
  }
  ...
  static insertUpdateActivityLevelData(
      ...
      DateTime activityEndDate, DateTime activityStartDate, List<HealthDataPoint> activityTypeData, int position) async {
    ...
    if (weekInsertedData.isEmpty) {
      ...
      if(weekInsertedData[0].key != null){
        weekInsertedData[0].iconPath = Utils.getNumberIconNameFromType(activityName);
        weekInsertedData[0].total = double.parse(getTotalMinFromTwoDates);
        weekInsertedData[0].activityStartDate = activityStartDate;
        weekInsertedData[0].activityEndDate = activityEndDate;
    ...
    if (caloriesData.isEmpty) {
      ...
      await addNewActivity(activityName,iconPath,"");
      await stepsHeartExEntry(dateStart, activityName, insertedId,activityStartDate,activityEndDate,position);
  ...
  static stepsHeartExEntry(DateTime dateTime,String activityName,int insertedId, DateTime activityStartDate,
      DateTime activityEndDate,int position) async {
  ...
  static exportDataFromHealth(Function callback) async {
    ...
    if(activityDataList.isNotEmpty) {
      ...
      List<HealthDataPoint> healthData = await health.getHealthDataFromTypes(
          startDateRead, endDateRead, (Platform.isAndroid)?Utils.getAllHealthTypeAndroid:Utils.getAllHealthTypeIos);
    ...
    try {
      ...
      if (activityDataList.isNotEmpty) {
        ...
        var dailyData = activityDataList
            .where((element) => element.type == Constant.typeDay)
            .toList()
            .where((element) => element.needExport)
            .toList();
