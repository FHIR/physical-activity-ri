...
class SmartAuthorizationCodeGrant implements AuthorizationCodeGrant {
  ...
  Future<Client> _handleAuthorizationCode(String? authorizationCode,{Function? callBackFunction}) async {
    ...
    String? idToken = newBody['id_token'];
    int? expiresIn = newBody['expires_in'];
    String? refreshToken = newBody['refresh_token'];

    Debug.printLog("accessToken..... $accessToken  idToken.......  $idToken");

    Preference.shared.setString(Preference.authToken,accessToken ?? "");
    Preference.shared.setString(Preference.idToken,idToken ?? "");
...
/*    if (idToken != null) {
    ...
    if(fhirParameters['practitioner_id'] != null){
      String smartFhirProviderId = fhirParameters['practitioner_id'] ?? "";
      Preference.shared.setString(Preference.smartFhirProviderId,smartFhirProviderId);

      if(fhirParameters['given_name'] != null && fhirParameters['family_name'] != null){
        String smartFhirPatientDisplayName = fhirParameters['given_name']+""+fhirParameters['family_name'] ?? "";
        Preference.shared.setString(Preference.smartFhirProviderName,smartFhirPatientDisplayName);
      }else{
        Preference.shared.setString(Preference.smartFhirProviderName,"");
      }
      // List<String> smartFhirGroupList = fhirParameters['group'] ?? [];

    }else{
      Preference.shared.setString(Preference.smartFhirProviderId,"");
      Preference.shared.setString(Preference.smartFhirProviderName,"");
    }

    if (fhirParameters['group'] != null) {
      var isAdmin = fhirParameters['group'].contains("admin");
      Preference.shared.setBool(Preference.smartFhirIsAdmin, isAdmin);
    }
    if (callBackFunction != null) {
      ...
      }else if(fhirParameters['group'].contains("patient") && fhirParameters['practitioner_id'] == null){
        map[Constant.isSuccess] = false;
        map[Constant.msg] = Constant.providerAcNotFound;
      }else{
        map[Constant.isSuccess] = false;
...
/*  set fhirParameters(Map<String, dynamic> parameters) {
  ...
  set fhirParameters(Map<String, dynamic> parameters) {
    ...
    fhirParameters['practitioner_id'] =
        parameters['practitioner_id'] ?? fhirParameters['practitioner_id'];
