...
class GraphController extends GetxController {
  ...
  // List<ChartData> chartHeartRate = [];
  // List<ChartData> chartHeartPeak = [];
  // List<ChartData> chartDateList = [];
  ...
  Future<void> onInit() async {
    ...
    if(Constant.listOfMeaSure.isNotEmpty){
      await onChangeActivityTimeAndFrameGraphData(selectedMeaSure,Constant.graphAMeaSureType);
    }
    update();
    super.onInit();
  }
  ...
  getFilteredData(String activity, String meaSure, String time, String timeFrame){
    ...
    if(activityTableData.isNotEmpty){
      ...
      if(time == Constant.timeWeek) {
        ...
        // var currentWeekStartDate = Utils.findFirstDateOfTheWeek(nowDate);
        ...
        if (timeFrame == Constant.frameLastWeek) {
          ...
          if(activity == Constant.titleNon) {
            ...
            var activityDataData = activityTableData.where((element) =>
            element.type == Constant.typeDaysData).toList();
            var activityDataweek = activityTableData.where((element) =>
            element.type == Constant.typeWeek).toList();
        ...
        List<ActivityTable> activityData = [];
        List<ActivityTable> meaSureDataList;
        if (activity == Constant.titleNon) {
          activityData = activityTableData.where((element) =>
          element.type == Constant.typeDay).toList();
          Debug.printLog("length....${activityData.length}");
        ...
        for (int i = 0; i < totalDays.length; i++) {
          List<ActivityTable> dataList = [];
          dataList = timeLevelDataList.where((element) =>
...
/*                  chartHeartRate.add(ChartData(
                      xAxisDateStr, 0.0, yRestHeart, 0.0));*/
...
/*                  chartHeartPeak.add(ChartData(
                      xAxisDateStr, 0.0, 0.0, yPeckHeart));*/
                }
              }
        ...
        // if(timeFrame == Constant.frameLifeTime /*|| timeFrame == Constant.frame6Months || timeFrame == Constant.frame3Months || timeFrame == Constant.frame1Year*/){
        if(timeFrame == Constant.frameLifeTime || meaSure == Constant.heartRate){
          if(meaSure == Constant.heartRate){
            if(!lineChart2 && lineChart3){
              chartDataList = chartDataList.where((element) =>
  ...
  ///   =================================================================================================================  ///
  /// This is use  a Dynamic TIme Frame in Use .....
